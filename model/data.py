            ...
            updatemenus = [
                dict(type="buttons",
                    buttons=[
                        ...
                        args=[None, {'frame':{'duration':40}, 'transition':{'duration':0}, 'fromcurrent':True}])])]),
                        
        frames = [
            go.Frame(
                data = [go.Scatter(
                    x = (df.index).tolist()[:k] if k != 0 else [(df.index).tolist()[k]],
                    y = df['mean oneacorn'].iloc[:k+1] if k != 0 else [df['mean oneacorn'].iloc[k]],
                    error_y = dict(
                        type = 'data',
                        symmetric = False,
                        array = df['max'].iloc[:k+1] if k != 0 else [df['max'].iloc[k]],
                        arrayminus = df['min'].iloc[:k+1] if k != 0 else [df['min'].iloc[k]]
