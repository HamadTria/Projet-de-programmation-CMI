...
sidebar = html.Div(
	[
		html.H2("Rendu Projet CMI", className="display-4", style={'font-family': 'system-ui'}),
		html.Hr(),
		html.P(
		...
		dbc.Nav(
			[
				dbc.NavLink("Distplot", href="/", active="exact"),
				dbc.NavLink("Animated bar chart", href="/animated_bar_chart", active="exact"),
				dbc.NavLink("Scatter matrix", href="/scatter_matrix", active="exact"),
...
def render_page_content(pathname):
	if pathname == "/":
		dropdown = view.GUI.build_dropdown_menu_multi_stations()
...
def update_distplot_chart(dims):
    sub_df = model.data.extract_df_distplot(dims)
    return view.GUI.build_figure_distplot(sub_df, dims)

@app.callback(
    Output("animated_bar_chart", "figure"),
    [Input("dropdown", "value")])
def update_bar_chart(value):
		sub_df, attributes = model.data.extract_df_animated_bar_chart(value)
